...
const NewTriggerActions = ({ newTrigger, soundFiles, selectedSound, selectedTimerExpirationSound, handleCheckboxChange, handleInputChange, handleSoundItemClick, handleTimerExpirationSoundItemClick, playSound }) => {
  const playSpeech = (speechText) => {
    window.electron.ipcRenderer.send("speak", speechText);
  };
  ...
  return (
    <section>
      ...
      <div className={`panel ${newTrigger.saySomething ? "" : "inactive"}`}>
        ...
        {newTrigger.saySomething && (
          <div className="speech-preview">
            <div className="input-row">
              <div className="play" onClick={() => playSpeech(newTrigger.speechText)}>
                <FontAwesomeIcon icon={faPlayCircle} />
              </div>
              <Input id="speechText" value={newTrigger.speechText} placeholder="" label="Say..." onTextChange={(value) => handleInputChange("speechText", value)} />
            </div>
            <span className="note small">The first synthesis of a phrase can take a couple seconds to generate.</span>
          </div>
        )}
      </div>
